...
chrome.storage.local.get(["validSchedules", "allTimeslots"], (result) => {
  if (chrome.runtime.lastError) {
    ...
    const scheduleNotification = document.getElementById(
...
filterForm.addEventListener("submit", function (event) {
  ...
  const unprefDays = [];
  document
    .querySelectorAll('input[name = "day-cb"]:not(:checked)')
    .forEach((checkbox) => {
...
function getCellForTimeSlot(timeslot) {
  ...
  if (existingTimeslot) {
    const conflictUnit = document.getElementById(
      `unitDropdown-${existingTimeslot.id.replace(/\s+/g, "")}`
    );
    conflictUnit.value = "";
  }
  ...
  timeslotDiv.style.backgroundColor =
    colorCode[`${timeslot.classType}${timeslot.description}`];
...
function analyseSchedules(validSchedules) {
  ...
  const allDays = [
    ...new Set(
      validSchedules.flatMap((schedule) =>
...
function createColorCode(timeslotGroups) {
  ...
  timeslotGroups.forEach((unit, index) => {
    ...
    if (index >= colorList.length) {
      colorCode[colorId] = `#${Math.floor(Math.random() * 16777215)
        .toString(16)
        .padStart(6, "0")}`;
    } else {
